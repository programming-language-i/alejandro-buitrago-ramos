...
R/: Concurrencia ya que las dos tareas se ejecutan en un mismo nucleo, lo que varia es el tiempo.

2. Nombrar dos diferencias entre un proceso y un hilo *(memoria, costo, fallo)*.

R/: crear procesos es más costoso en recursos y si un proceso falla no afecta a otros; los hilos son más ligeros, pero si uno falla puede comprometer todo el proceso.

3. ¿En qué estado del ciclo de vida está un hilo que ejecuta `time.sleep(2)`? ¿Qué devuelve `is_alive()`?

R/:El hilo está en estado bloqueado (sleeping), no ejecuta código durante ese tiempo.
is_alive() devuelve True, porque el hilo sigue existiendo aunque esté dormido.

4. ¿Qué protege el GIL? ¿Evita las condiciones de carrera en los datos del programa?

R/: protege el acceso a los objetos internos de CPython, garantizando que solo un hilo ejecute bytecode de Python a la vez.

No evita condiciones de carrera en los datos del programa: si compartes estructuras entre hilos, debes usar tus propios mecanismos de sincronización locks, semáforos, etc.

5. ¿Por qué 4 hilos que duermen 1 s cada uno tardan ~1 s en total y no ~0,25 s?

R/: Porque dormir no consume CPU, y los hilos se ejecutan concurrentemente. Todos entran en sleep al mismo tiempo y despiertan juntos, así que el tiempo total es el del mayor sleep (≈1 s), no la suma (≈4 s).

6. Al crear un hilo por herencia, ¿qué método se sobrescribe y cuál nunca? ¿Por qué?

R/: Al crear un hilo por herencia de Thread, se sobrescribe run() para definir la tarea.

Nunca se sobrescribe start(), porque este método gestiona la creación interna del hilo en el sistema operativo. Si lo modificas, rompes el ciclo de vida correcto

7. ¿Qué pasa con un hilo daemon cuando termina el hilo principal? ¿Qué **no** se ejecuta?

R/:Cuando el hilo principal termina, los hilos daemon se matan automáticamente.

Lo que no se ejecuta son sus bloques finally o limpieza final: no hay garantía de que terminen su trabajo. 

8. Completar la regla del curso: *hilos para , procesos para*.

R/: Hilos para concurrencia, procesos para paralelismo.

Hilos: útiles para tareas que esperan (I/O, dormir, red).

Procesos: útiles para cálculos pesados que necesitan varios núcleos.
